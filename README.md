...
- **截屏翻译**：`⌥S` 唤起框选（每屏遮罩、尺寸提示、ESC 取消），松开自动 OCR（Apple Vision，端上离线）并翻译
- **重截上次区域**：`⌥⇧S` 直接按上次选区截图，适合字幕/日志等连续翻译
- **原文/译文对照**：原图 / 译文 / 并排三种视图切换
- **屏幕置顶**：结果面板常驻最上层（可关），失焦自动降透明度（可调），拖动、缩放、位置记忆
- **选词收藏**：译文或原文中选中文字右键「收藏到生词本」，工具条「收藏选中」按钮同样可用；生词本支持搜索、删除、CSV 导出（UTF-8 BOM）
- **一键复制** / 快速关闭
- **菜单栏驻留**：关闭窗口不退出，菜单栏访问全部功能，可开机自启
